...
module.exports = ({ strapi }) => ({
    async getTranslatableData(sourceData, isComponentData) {
    ...
    async translate(
        ...
        try {
            ...
            for (let language of langs) {
                ...
                const isLocalization = localizations.filter((locale) => locale.locale === language);
    ...
    async initialize() {
        const strapiSetting = await strapi.config.get("plugin.strapi-google-translator");
    ...
    async doTranslation(dataToTranslate, sourceLocale, targetLocale, collectionName, token) {
        try {
    ...
    async saveTranslation(backendUrl, collectionTypeSlug, sourceId, translatedFields, apiToken) {
        try {
            ...
            console.error(error);
            throw new ApplicationError(`Unable to save translation : ${error.message} `);
        }
    },

    // Update translated data
    async updateTranslation(backendUrl, collectionTypeSlug, id, translatedFields, token, targetLocale) {
        try {
            ...
            console.error(error);
            throw new ApplicationError(`Unable to update translation : ${error.message} `);
        }
    },
});
