...
module.exports = ({ strapi }) => ({
  ...
  async  getTranslatorInstance() {
    ...
    if(!translatorInstance){
      try {
        ...
        const googleEnv = JSON.parse(googleJson);
        translatorInstance = new Translate({
          credentials: googleEnv,
          projectId: googleEnv.project_id,
        });
        ...
        throw new ApplicationError(
          `Unable to create translator instance`
        );
       }
    }
  ...
  async translateApiCall(text, locale) {
    let response;
    try {
      let translatorInstance = await this.getTranslatorInstance();
          const [translation] = await translatorInstance.translate(text, locale);
          const result = this.removeIgnoreTag(translation);
          response = result;
      return response;
    } catch (error) {
  ...
  async getTranslationStrategy(fieldName, schema, dataToTranslate, glossaries, locale) {
  ...
  async skipTranslate(data, componentType, locale) {
